...
/**
 * Iterates through a range of standard MIDI notes (C2 to C5) to find which 
 * major scale best matches the set of 8 detected frequencies by minimizing 
 * total cents deviation.
 */
export function findBestFitScale(detectedFreqs: number[]): number {
  let bestTonicFreq = 0;
  let minTotalError = Infinity;

  // Search range: MIDI 36 (C2) to 72 (C5)
  for (let midi = 36; midi <= 72; midi++) {
    ...
    for (let i = 0; i < 8; i++) {
      if (detectedFreqs[i] > 0) {
        totalError += Math.abs(calculateCents(detectedFreqs[i], candidateScale[i]));
      } else {
        // Penalty for silence/missing notes
        totalError += 200; 
      }
    }
