          ...
          {entries.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-16 text-center">
              <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-slate-300" viewBox="0 0 24 24" fill="currentColor"><path d="M19 5h-2V3H7v2H5c-1.1 0-2 .9-2 2v1c0 2.55 1.92 4.63 4.39 4.94.49.73 1.16 1.32 1.94 1.7L8.33 19H7c-.55 0-1 .45-1 1s.45 1 1 1h10c.55 0 1-.45 1-1s-.45-1-1-1h-1.33l-1-4.36c.78-.38 1.45-.97 1.94-1.7C19.08 12.63 21 10.55 21 8V7c0-1.1-.9-2-2-2z"/></svg>
              </div>
              <p className="text-sm font-black text-slate-400">No entries yet</p>
              <p className="text-xs text-slate-300 font-medium mt-1">Complete a test and add your name to start the board.</p>
            </div>
          ) : (
            entries.map((entry, i) => (
              <div
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${entry.isUser ? 'text-indigo-200' : 'text-slate-400'}`}>
                      {entry.scalesCompleted}/8 Notes • {entry.date}
                    </span>
                  </div>
                </div>
